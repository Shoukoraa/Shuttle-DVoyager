...
<body class="bg-[#F9FAFB] text-dark-800 font-sans antialiased overflow-x-hidden min-h-screen flex flex-col md:flex-row">
    ...
    <aside id="sidebar" class="fixed inset-y-0 left-0 transform -translate-x-full md:translate-x-0 transition-transform duration-300 ease-in-out md:sticky md:top-0 md:flex flex-col w-64 h-screen bg-dark-900 text-white z-40 shadow-xl flex-shrink-0 border-r border-dark-850">
        ...
        <nav class="flex-grow py-6 px-4 space-y-1.5 overflow-y-auto custom-scrollbar">
            ...
            <!-- Vouchers / Promos -->
            <a href="{{ route('admin.vouchers') }}" class="group flex items-center px-4 py-3 text-sm font-medium rounded-xl transition-all duration-200 {{ request()->routeIs('admin.vouchers*') ? 'bg-[#2D2D2D] text-brand-500 border-l-4 border-brand-500 shadow-inner font-semibold' : 'text-gray-400 hover:text-white hover:bg-dark-850' }}">
                <svg class="w-5 h-5 mr-3 transition-colors {{ request()->routeIs('admin.vouchers*') ? 'text-brand-500' : 'text-gray-400 group-hover:text-white' }}" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
            ...
            <!-- Chat CS -->
            <a href="{{ route('admin.chat') }}" class="group flex items-center px-4 py-3 text-sm font-medium rounded-xl transition-all duration-200 {{ request()->routeIs('admin.chat*') ? 'bg-[#2D2D2D] text-brand-500 border-l-4 border-brand-500 shadow-inner font-semibold' : 'text-gray-400 hover:text-white hover:bg-dark-850' }}">
                <svg class="w-5 h-5 mr-3 transition-colors {{ request()->routeIs('admin.chat*') ? 'text-brand-500' : 'text-gray-400 group-hover:text-white' }}" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
