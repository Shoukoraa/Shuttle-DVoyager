...
<div class="space-y-8 animate-fade-in">
    ...
    <div class="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm flex items-center justify-between">
        <div>
            <h1 class="text-2xl font-extrabold font-outfit text-dark-900 leading-tight">Master Kendaraan</h1>
            <p class="text-sm text-gray-500 mt-1">Kelola data armada kendaraan, kapasitas kursi, dan status perawatan Shuttle D-Voyager.</p>
        </div>
        <div class="h-12 w-12 rounded-xl bg-brand-500/10 border border-brand-500/20 flex items-center justify-center text-brand-600">
            <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16H9m8-3v-3a2 2 0 00-2-2H9a2 2 0 00-2 2v3m12 0h1a1 1 0 011 1v2a1 1 0 01-1 1h-1m-12 0H4a1 1 0 01-1-1v-2a1 1 0 011-1h1m10-5h4l-2-3h-2v3zM7 9h3V6H8a1 1 0 00-1 1v2z" />
            </svg>
        </div>
    </div>
        ...
        <div class="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm h-fit">
            ...
            <form action="/admin/vehicles" method="POST" class="space-y-4">
                ...
                <div class="flex flex-col gap-1.5">
                    ...
                    <input type="text" name="plate_number" id="plate_number" placeholder="Contoh: D 1234 AB" required class="w-full bg-gray-50 border border-gray-200 text-dark-900 text-sm rounded-xl px-4 py-2.5 focus:border-brand-500 focus:ring-1 focus:ring-brand-500 outline-none transition-all font-medium">
                </div>

                <div class="flex flex-col gap-1.5">
                    <label for="vehicle_type" class="text-xs font-bold text-gray-500 uppercase tracking-wider">Tipe / Model</label>
                    <input type="text" name="vehicle_type" id="vehicle_type" placeholder="Contoh: Toyota Hiace Commuter" required class="w-full bg-gray-50 border border-gray-200 text-dark-900 text-sm rounded-xl px-4 py-2.5 focus:border-brand-500 focus:ring-1 focus:ring-brand-500 outline-none transition-all font-medium">
                </div>
        ...
        <div class="lg:col-span-2 bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
            <div class="px-6 py-5 border-b border-gray-50 flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    ...
                    <p class="text-xs text-gray-400">Total terdaftar: {{ $vehicles->total() }} armada aktif.</p>
                </div>
            ...
            @if($vehicles->isEmpty())
                <div class="flex flex-col items-center justify-center p-12 text-center space-y-3">
                    ...
                    <div class="space-y-1">
                        <h4 class="text-sm font-semibold text-dark-900">Belum Ada Armada</h4>
                        <p class="text-xs text-gray-400 max-w-xs">Tambahkan kendaraan baru pada formulir di sebelah kiri.</p>
                    </div>
                </div>
            @else
                    ...
                    <table class="w-full text-left border-collapse">
                        <thead>
                            <tr class="bg-gray-50 border-b border-gray-100">
                                ...
                                <th class="px-6 py-3.5 text-xs font-bold text-gray-400 uppercase tracking-wider text-center">Kapasitas</th>
                            ...
                            @foreach($vehicles as $veh)
                                <tr class="hover:bg-gray-50/50 transition-colors">
                                    ...
                                    <td class="px-6 py-4 text-center whitespace-nowrap">
                                        @php
                                            $vStatus = strtolower($veh->status);
                                            $vBadge = 'bg-gray-50 text-gray-600 border-gray-200';
                                            if ($vStatus === 'active' || $vStatus === 'aktif') {
                                                $vBadge = 'bg-emerald-50 text-emerald-700 border-emerald-200';
                                            } elseif ($vStatus === 'maintenance' || $vStatus === 'servis') {
                                                $vBadge = 'bg-amber-50 text-amber-700 border-amber-200';
                                            } elseif ($vStatus === 'inactive' || $vStatus === 'nonaktif') {
                                                $vBadge = 'bg-rose-50 text-rose-700 border-rose-200';
                                            }
                                        @endphp
                                        <span class="inline-flex items-center px-2.5 py-0.5 text-xs font-semibold rounded-lg border {{ $vBadge }} uppercase tracking-wider">
                                            {{ $veh->status }}
                                        </span>
                                    </td>
                                    <td class="px-6 py-4 text-center whitespace-nowrap">
                                        <div class="inline-flex items-center gap-2">
                                            <a href="/admin/vehicles/{{ $veh->id }}/edit" class="inline-flex items-center justify-center px-3 py-1.5 bg-brand-500 hover:bg-brand-600 text-dark-900 text-xs font-bold rounded-lg transition-colors gap-1 shadow-sm">
                                                <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            ...
                                            <form action="/admin/vehicles/{{ $veh->id }}" method="POST" onsubmit="return confirm('Hapus kendaraan ini?')">
                                                @csrf
                                                @method('DELETE')
