        ...
        <div class="lg:col-span-2 bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
            <div class="px-6 py-5 border-b border-gray-50 flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    ...
                    <p class="text-xs text-gray-400">Total terdaftar: {{ $locations->total() }} lokasi.</p>
                </div>
                
                <!-- Action Bar -->
                <div class="flex items-center gap-2">
                    <form id="bulkDeleteForm" action="{{ route('admin.locations.bulk-delete') }}" method="POST" class="inline">
                        @csrf
                        <button type="submit" onclick="return confirm('Hapus semua lokasi terpilih?')" class="inline-flex items-center justify-center px-3.5 py-1.5 bg-rose-50 hover:bg-rose-100 text-rose-700 text-xs font-bold rounded-lg border border-rose-150 transition-colors gap-1.5 shadow-sm">
                    ...
                    <form action="{{ route('admin.locations.delete-all') }}" method="POST" class="inline">
                        @csrf
                        <button type="submit" onclick="return confirm('Apakah Anda yakin ingin menghapus SEMUA lokasi? Tindakan ini tidak bisa dibatalkan.')" class="inline-flex items-center justify-center px-3.5 py-1.5 bg-dark-900 hover:bg-[#A32A2A] text-white text-xs font-bold rounded-lg transition-colors gap-1.5 shadow-sm border border-dark-900">
                            Bersihkan Semua
                        </button>
                    </form>
                </div>
                            ...
                            @foreach($locations as $loc)
                                <tr class="hover:bg-gray-50/50 transition-colors">
                                    <td class="px-5 py-4 text-center whitespace-nowrap">
                                        <input form="bulkDeleteForm" type="checkbox" name="location_ids[]" value="{{ $loc->id }}" class="item-checkbox rounded text-brand-500 focus:ring-brand-500 h-4 w-4 border-gray-300">
                                    </td>
